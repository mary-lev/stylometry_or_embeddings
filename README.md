...
**Embeddings (six models on Zenodo):**
...
prints against the *Expected* block underneath. Runtimes are from one 20-core
machine.
...
For Russian the null is dense right at the residual, so the exact p moves with
the residual's second decimal (3.78 / 3.80 / 3.86% give p = 0.089 / 0.080 /
0.058); all are above 0.05 and inside the CI, so the conclusion is unaffected.
`--recompute` redoes the permutations from scratch (~3.5 h Russian, ~7 h
Italian) and is not needed.
...
This is the one result computed from shipped per-poem distances rather than
from the corpus, because the perturbed texts are generated at run time.
`--regenerate` re-embeds them through the Gemini API (1,450 paid calls) and is
not needed.
...
both read `method_differences_gemini.json`.
